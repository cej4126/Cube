   ...
   m_AABB({0, 0, 0}, {0, 0, 0})
{
   ...
   m_AlignedData->m_InversrTransform = XMMatrixInverse(nullptr, localTransform);
}
...
DirectX::XMMATRIX SceneNode::GetInverseTransform() const
{
   return m_AlignedData->m_InversrTransform;
}
...
void SceneNode::AddChild(std::shared_ptr<SceneNode> childNode)
{
   if (childNode)
   {
      std::vector<std::shared_ptr<SceneNode>>::iterator iter = std::find(m_Children.begin(), m_Children.end(), childNode);
...
void SceneNode::RemoveChild(std::shared_ptr<SceneNode> childNode)
{
   if (childNode)
   {
      std::vector<std::shared_ptr<SceneNode>>::const_iterator iter = std::find(m_Children.begin(), m_Children.end(), childNode);
      if (iter != m_Children.cend())
      {
         ...
         std::multimap<std::string, std::shared_ptr<SceneNode>>::iterator iter2 = m_ChildrenByName.find(childNode->GetName());
...
size_t SceneNode::AddMesh(std::shared_ptr<Mesh> mesh)
{
   ...
   if (mesh)
   {
      std::vector<std::shared_ptr<Mesh>>::const_iterator iter = std::find(m_Meshes.begin(), m_Meshes.end(), mesh);
...
std::shared_ptr<Mesh> SceneNode::GetMesh(size_t index)
{
